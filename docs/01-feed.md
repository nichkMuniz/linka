...
> - Posts de usuários bloqueados são filtrados no feed e em Descobrir — e,
>   desde 14/09/2026, **também no ring de flows** (`getActiveStoriesDb` cruza
>   `getFollowingIdsDb` com `getBlockedIdsDb`, como `getFeedPosts` já fazia).
>   **Bug corrigido junto:** o trigger `user_blocks_unfollow_trg` desfazia o
>   follow apenas em `followers`, tabela que o app não lê — o vínculo em
>   `following` sobrevivia e o bloqueado continuava no ring, marcado como
>   seguido, enquanto os posts dele sumiam (o que escondia a causa). Ver
>   `docs/migrations/20260914-block-unfollow-following.sql` — **precisa ser
>   rodada**. O cache de módulo do feed (`feedCache`) também é derrubado por um
>   listener de escopo de módulo no evento `ritmofit-blocks-changed`, disparado
>   pelo `BlockUserDialog`: bloquear de **fora** do feed (perfil, flow viewer)
>   não passava por reload nenhum, e voltar para a home restaurava a lista
>   antiga com o bloqueado ainda lá.
...
- Captura pela câmera com **obturador inteligente**: **toque rápido = foto**, **segurar (>400ms) = grava vídeo** (estilo Instagram/Snapchat). Solta o obturador para finalizar o vídeo
  ...
  - **Câmera abrindo com zoom enorme (corrigido em 14/09/2026):** o `getUserMedia` voltou a pedir o preset **landscape** (`width: 1920`, `height: 1080`, 30fps) e **não pede mais `aspectRatio`/retrato**. O WebKit do iOS não tem modo de sensor 9:16: para satisfazer a restrição ele pega o preset mais próximo, **recorta o centro do frame** e reescala o pedaço até o tamanho pedido — ou seja, a restrição de enquadramento virava zoom óptico aparente já na abertura da câmera (e uma imagem mais mole, por ser um recorte ampliado). O 9:16 continua existindo, só que onde não custa campo de visão: `centerCrop` no canvas da gravação e `object-cover` no preview/viewer — então o ganho de nitidez de 27/08 segue valendo. No mesmo passo, o nível de zoom da pinça passa a ser **lido da track** (`getSettings().zoom`) em vez de assumido como 1: iPhones com câmera virtual (dupla/tripla) abrem com fator nativo maior que 1, e o primeiro gesto de pinça saltava o enquadramento para um valor que o usuário nunca escolheu.
...
- **Copiar meta** — Se o post tiver meta, botão para copiar para o próprio perfil. Depois de copiar, o botão fica **travado** e vira "Meta copiada" (ver "Botão Copiar Meta trava depois de copiar" abaixo)
...
- **Botão "Copiar Meta" trava depois de copiar (2026-09-14):** ao copiar a meta pelo drawer de progresso, o botão passa a **desabilitado**, em estilo outline com ✓ verde e a label **"Meta copiada"** (`feed_goal_already_copied`). Duas mudanças sustentam isso:
  - O drawer **não fecha mais** ao copiar com sucesso (antes fechava logo após o toast, e o estado copiado nunca chegava a ser visto).
  - O estado **não vive só na memória do drawer**: ao abrir, quando o post é de outra pessoa, o `openGoalModal` consulta `getUserGoalsDb()` e trava o botão se o visitante já tiver uma `user_goals` com aquele `goal_id`. Antes, `hasAlreadyCopiedGoal` era zerado a cada abertura, então fechar e reabrir o drawer liberava copiar **a mesma meta de novo**, criando linhas duplicadas em `user_goals`. Como a checagem é por `goal_id`, uma meta que o usuário já tinha por conta própria também aparece como copiada — proposital, é o mesmo efeito de duplicar.
  - Sem rede, a consulta falha em silêncio e o botão segue liberado (a cópia em si é que vai falhar, com toast de erro).
  - Cada abertura do drawer recebe um número (`goalModalReqRef`): respostas de uma abertura anterior — tanto a checagem de cópia quanto `getRoutinesByGoalIdDb` — são descartadas, para não pintarem o estado de uma meta que não está mais na tela.
