...
- **Resposta privada a um flow (2026-08-17):** prefixo `[flowreply]:<flowId>|<texto>` → renderizado por `FlowReplyMessage` (`components/community/`) como uma bolha estilo Instagram: rótulo de contexto ("Respondeu ao seu flow" / "Você respondeu ao flow"), **miniatura vertical do flow** (68×104) e, abaixo, o texto que a pessoa digitou. Origem: o botão de mensagem na doca do `FlowViewer` (ver `docs/01-feed.md`) — o comentário não vira balão público, vai direto para a caixa de mensagens do autor.
  - Parser e builder ficam em `client/lib/flow-reply.ts` (`buildFlowReplyPayload` / `parseFlowReply`). O separador é o **primeiro** `|`: o `flow.id` é bigint e nunca contém `|`, então um `|` digitado no texto sobrevive. Payload malformado (sem separador/id) volta `null` e a bolha cai no render de texto puro em vez de exibir um card quebrado.
  - **O texto é sempre renderizado, mesmo sem a miniatura.** Flow expira em 24h e pode ser apagado pelo autor; a mensagem é uma DM como outra qualquer e não some junto com a mídia. Flow apagado → miniatura "Conteúdo indisponível"; flow > 24h → miniatura escurecida com "Flow expirado" e **sem** navegação (o ring já não o abriria). Dentro das 24h, tocar navega para `/flows/:id`.
  - Vídeo usa `poster_url` quando existe (imagem leve); sem capa, cai no fragment `#t=0.1` (`videoPosterSrc`) para o WebView pintar o 1º frame. Flow só-texto mostra o `background_color` com a legenda.
  - `getFlowByIdDb` não é cacheado e faz duas queries; o componente mantém um **memo de sessão por flow id** (máx. 50 entradas, falha não fica grudada) para que várias respostas ao mesmo flow não repitam a busca.
- Em previews (última mensagem na lista de conversas, citação de reply e banner de resposta), mensagens especiais exibem rótulo curto traduzido em vez do texto bruto: `🎤 Áudio`, `🖼️ Imagem`, `📤 Post`, `🎬 Shot` (helper `specialMessageLabel` em `community-helpers.ts`). A resposta a flow é a exceção: o preview mostra `🎞️ <o texto digitado>` (é o que interessa na lista), caindo no rótulo `🎞️ Resposta ao flow` só quando o texto vem vazio
...
**Aviso fora da Comunidade (2026-08-06):** com o app aberto em qualquer outra tela (feed, metas, perfil…), a mensagem recebida dispara **vibração leve** (`hapticLight`) + um **pop up in-app** com avatar, apelido e preview — `IncomingMessageToast`, montado no `AppLayout` e alimentado pelo canal realtime `app-layout-messages`. Tocar abre a conversa (`/comunidade?user=<remetente>`). Dentro da conversa daquele remetente o pop up é **suprimido** (a mensagem já aparece na tela) e resta só a vibração; quem publica a conversa aberta é `setActiveConversationUserId` (`client/lib/active-conversation.ts`), gravado/limpo pela `Community`. Com o app em background continua valendo o push do iOS. Detalhes e o porquê de o gatilho ser `messages` (e não a notificação tipo 10) em `docs/10-notificacoes.md`.
...
- Seletor de "O que você treinou?" — exibe **apenas rotinas concluídas** nos últimos 7 dias (via `getRecentCompletedRoutinesDb`)
  ...
  - **Uma execução = um card (2026-08-06):** o agrupamento das linhas de `user_workouts_hist` é feito **por sessão** — todas as séries gravadas num mesmo "Finalizar" recebem `sessionBaseMs + índice` em ms, então linhas a menos de 60s uma da outra são a mesma execução (mesma premissa de `getLastWorkoutSessionSeriesDb`). Sessões da **mesma rotina no mesmo dia local** ainda se fundem num card só, que é o par usado pela regra de duplicidade abaixo. O nome sai de `user_workouts.name`, com reserva em `routines.name` (via `user_workouts_hist.routine_id`) e, só então, "Rotina de Exercícios". <br>**Bug corrigido:** o agrupamento anterior era por *nome + dia UTC*, então uma única execução aparecia **duas vezes** no seletor sempre que parte das séries não resolvia o nome — o caso comum é o **exercício avulso adicionado durante o treino**, que grava `user_workout_id` NULL e virava um segundo card "Rotina de Exercícios". O dia em UTC também jogava treinos noturnos (a partir das 21h no Brasil) para o dia seguinte, separando execuções do mesmo dia.
...
>
> **Atualização 2026-08-14:** `cdnImg` virou pass-through (a foto agora sobe já em 1440px — ver *Pipeline de imagens* em `docs/13-layouts-e-componentes.md`), então card e modal pedem **a mesma URL**: o cache já está quente por natureza. O prefetch continua no lugar e continua útil (aquece o objeto no CDN para quem ainda não rolou até o card), só não está mais compensando um segundo endpoint.
...
- **Abrir um check-in específico:** `openCheckInById(checkInId)` carrega detalhe + comentários + reações, **carrega também o grupo por trás** (`getDuelGroupDb` → `openGroupView`) e abre o drawer na aba Duelos. Dois caminhos chegam nela: `state.openCheckIn` (navegação interna, vinda do card de notificação) e **`?checkin=<id>`** (toque no push — um deep link é só uma URL e não carrega o `state` do router). O param é aplicado uma única vez por id, via ref
  - **Por que carregar o grupo junto (2026-07-31):** a barra de aprovar/reprovar do modo memes só renderiza quando `selectedGroupForView?.scoringType === "memes"` **e** `checkInVotes` está carregado — os dois são preenchidos por `openGroupView`. Abrindo o check-in direto pela notificação sem isso, o drawer aparecia **sem os botões de avaliação**; eles só voltavam ao fechar e reabrir pela tela do grupo. Chamar `openGroupView` também deixa o grupo carregado por trás, então fechar o drawer cai no grupo certo em vez de numa tela vazia. `openGroupView` acrescenta `?group=<id>` à URL, mas o efeito de restauração de grupo não reabre nada (guarda por `selectedGroupForView` já setado)
