...
/** Estado + ações de comentários de uma promoção — compartilhado entre o drawer
...
type CommentsState = ReturnType<typeof usePromotionComments>;

/** Lista de comentários (skeleton / itens / vazio). `className` controla se ela
 * ganha altura fixa + scroll próprio (drawer dedicado) ou flui junto com o
 * container pai (seção embutida no drawer de detalhe). */
function PromotionCommentsList({
  state,
  className,
}: {
  state: CommentsState;
...
/** Seção de comentários embutida — usada dentro do drawer de detalhe da
 * promoção, sem drawer/scroll próprios (flui junto com o conteúdo ao redor). */
export function PromotionCommentsSection({ promotionId }: { promotionId: string }) {
